...
class LiveDashboard {
    constructor() {
        this.data = {
            combinedTarget: 0, // Will be fetched from API
            teams: {
                eli: {
                    name: 'ELI',
                    disbursementTarget: 0, // Will be fetched from API
...
class APIIntegration {
    constructor() {
        ...
        // CORS proxy options (uncomment if needed)
        this.corsProxies = [
            '', // Try direct first
            'https://corsproxy.io/?',
    ...
    async fetchWithProxy(url, proxyIndex = 0) {
        ...
        try {
            ...
            // If CORS error and we have more proxies to try
    ...
    updateDashboard(eliData, nblData) {
        // Calculate combined target from both APIs
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Wait a moment before starting API calls
    setTimeout(() => {
        new APIIntegration();
    }, 1000);
});
